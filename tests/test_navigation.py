...
class TestNavigation(unittest.TestCase):
    ...
    def _complete_transition(self):
        """Helper to complete a pending map transition immediately."""
        if self.world.pending_warp:
            warp = self.world.pending_warp
            self.world._execute_map_change(warp.target_map, warp.target_x, warp.target_y, self.player)
            self.world.pending_warp = None
            self.world.map_transition_active = False
            self.world.map_transition_phase = "none"
    ...
    def test_route1_to_pallet_transition(self):
        ...
        # Start in Route 1
        self.world.change_map("route_1", 20, 38, self.player, immediate=True)
        self.assertEqual(self.world.current_map_id, "route_1")
