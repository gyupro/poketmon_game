...
class RepelItem(Item):
    """Item that prevents wild Pokemon encounters."""
    
    def __init__(self, item_id: str, name: str, description: str, 
                 price: int, steps: int):
        super().__init__(item_id, name, description, ItemCategory.FIELD, price,
                        can_use_in_battle=False)
        self.steps = steps
    ...
    def use(self, target: Optional[Pokemon] = None, **kwargs) -> Tuple[bool, str]:
        ...
        if not world:
            return False, "Cannot use this item here!"
        
        world.use_repel(self.steps)
        return True, f"You used {self.name}! Wild Pokemon will be repelled for {self.steps} steps."
...
ITEM_REGISTRY: Dict[str, Item] = {
    ...
    # Repel items
    "repel": RepelItem("repel", "Repel", "Repels weak wild Pokemon for 100 steps", 350, 100),
    "super_repel": RepelItem("super_repel", "Super Repel", "Repels weak wild Pokemon for 200 steps", 500, 200),
    "max_repel": RepelItem("max_repel", "Max Repel", "Repels weak wild Pokemon for 250 steps", 700, 250),
}
